    ...
    <div class="success-container">
      ...
      <div class="success-details">
        ...
        <div class="success-detail-row">
          ...
          <span class="success-text-preview" style="font-family:'${state.font}',sans-serif;">${state.text}</span>
        </div>
        <div class="success-detail-row">
          <span>${t('font')}</span>
...
function init3DCupPreview(container, cupConfig) {
  ...
  function drawTexture(text, fontId, bg, textColor, patternSrc) {
    ...
    // === Preview zone ===
    // Strictly 1/4 of the cup's arc (centered), 1/3 from top & 1/3 from bottom
    const previewLeft   = Math.round(w / 2 - w / 8);   // centered 1/4 arc
    const previewRight  = Math.round(w / 2 + w / 8);
    const previewTop    = Math.round(h / 3);            // 1/3 from top
    const previewBottom = Math.round(h * 2 / 3);        // 1/3 from bottom
    const previewW = previewRight - previewLeft;
    ...
    // Subtle shading gradient for depth illusion (full width)
    ...
    // Dim non-preview areas to clearly define the preview window
    ctx.fillStyle = 'rgba(0,0,0,0.14)';
    ctx.fillRect(0, 0, w, previewTop);                         // top strip
    ctx.fillRect(0, previewBottom, w, h - previewBottom);      // bottom strip
    ctx.fillRect(0, previewTop, previewLeft, previewH);        // left strip (mid)
    ctx.fillRect(previewRight, previewTop, w - previewRight, previewH); // right strip (mid)

    // Preview window border (subtle dashed)
    ctx.save();
    ctx.strokeStyle = 'rgba(255,255,255,0.18)';
    ctx.lineWidth = 1;
    ctx.setLineDash([4, 4]);
    ctx.strokeRect(previewLeft + 1, previewTop + 1, previewW - 2, previewH - 2);
    ctx.restore();

    // Pattern image (clipped to preview zone)
    if (_patternLoaded && _patternImg) {
      ctx.save();
      ctx.beginPath();
      ctx.rect(previewLeft, previewTop, previewW, previewH);
      ctx.clip();
      ctx.globalAlpha = 0.35;
      ctx.drawImage(_patternImg, previewLeft, previewTop, previewW, previewH);
      ctx.globalAlpha = 1.0;
      ctx.restore();
    }

    // Text (multi-line wrapped, clipped to preview zone)
    const displayText = text || 'PREVIEW';
    // Robust font stack: includes emoji/symbol fallbacks for kaomoji characters
    const canvasFontStack = "'Noto Sans SC', 'Segoe UI Symbol', 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI', sans-serif";
    const fontFamily = fontId ? `'${fontId}', ${canvasFontStack}` : canvasFontStack;

    // Determine font size that fits both width and height
    // 15% safety margin: ctx.measureText is inaccurate for special Unicode/emoji
    const maxTextW = Math.floor((previewW - 16) * 0.85);
    const maxTextH = previewH - 16;
    const { fontSize, lines } = calcTextLayout(ctx, displayText, fontFamily, maxTextW, maxTextH);
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(previewLeft + 4, previewTop + 4, previewW - 8, previewH - 8);
    ctx.clip();
...
/**
 * Calculate optimal font size and wrapped lines to fit text
 * within maxWidth × maxHeight px on a canvas context.
 * Returns { fontSize: number, lines: string[] }
 */
function calcTextLayout(ctx, text, fontFamily, maxWidth, maxHeight) {
  var len = (text || '').length;
  if (len === 0) return { fontSize: 24, lines: ['PREVIEW'] };

  // Determine if mostly CJK (character-based wrapping)
  var cjkCount = (text.match(/[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/g) || []).length;
  var isCJK = cjkCount >= len * 0.5;

  // Start with a generous size, then shrink until text fits
  var fontSize = isCJK ? 28 : 22;
  if (len <= 3) fontSize = isCJK ? 32 : 26;
  else if (len <= 6) fontSize = isCJK ? 28 : 22;
  else if (len <= 10) fontSize = isCJK ? 24 : 18;
  else if (len <= 15) fontSize = isCJK ? 20 : 15;
  else if (len <= 20) fontSize = isCJK ? 17 : 13;
  else fontSize = isCJK ? 14 : 11;

  var lines, lineHeight, totalH;

  while (fontSize >= 9) {
    ctx.font = 'bold ' + fontSize + 'px ' + fontFamily;
    lines = wrapLines(ctx, text, maxWidth, isCJK);
 ...
 * CJK text: break at any character.
 * Non-CJK: break at word boundaries (spaces).
 */
function wrapLines(ctx, text, maxWidth, isCJK) {
  var lines = [];
  if (!text || text.length === 0) return ['PREVIEW'];
  ...
  if (isCJK) {
    ...
    // Word-based for Latin text
    var words = text.split(' ');
    var line2 = '';
    for (var j = 0; j < words.length; j++) {
      var testLine2 = line2 ? line2 + ' ' + words[j] : words[j];
      if (ctx.measureText(testLine2).width > maxWidth && line2.length > 0) {
        lines.push(line2);
        line2 = words[j];
      } else {
        line2 = testLine2;
  ...
  return lines.length > 0 ? lines : ['PREVIEW'];
}
